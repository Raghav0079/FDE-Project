...
if AGENT_LLM_SETTING == "OPENAI":
    ...
    print(
        "🐳 Brain Mode: Utilizing Flagship DeepSeek Cloud Reasoner (deepseek-v4-pro)..."
    )
    from langchain_openai import ChatOpenAI
    ...
    print(
        "🤗 Brain Mode: Local Fallback Activated. Binding Local Ollama (qwen2.5:7b)..."
    )
    # Updated import from dedicated langchain_ollama package
...
# ==========================================
# 4. HELPER EXECUTION WRAPPER
# ==========================================
def run_orchestrator(query: str, thread_id: str = "production_test_1") -> str:
    """Invokes the LangGraph orchestrator state machine with a given prompt."""
    ...
    # Initialize thread state with system prompt
    fde_agent.invoke({"messages": [system_prompt]}, config=thread_config)

    # Run user query through state graph
    result = fde_agent.invoke({"messages": [("user", query)]}, config=thread_config)
